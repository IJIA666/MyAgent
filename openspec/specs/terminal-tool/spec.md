...
此外，当系统处于 `plan` 权限模式下时，前置安全判定层必须（MUST）与执行期结构校验层在允许集合上保持严格同构：任何会被执行期结构校验拒绝的复合字符或注入模式，也必须在 `plan` 模式的前置判定中被识别并拒绝，确保不存在"审批通过但执行失败"的路径。只有满足原子化结构约束且命中只读白名单前缀的命令，才允许进入后续审批流程。
