# agent-event-lifecycle Specification

## Purpose

定义智能体事件生命周期契约：`complete` 事件是每轮推理的唯一状态终结点，`error` 事件不影响 CLI 渲染状态机，斜杠命令以 stdin 独占事务执行，InputListener 的 pending resume 可取消。后台子代理的 `task_update` 是非终结性状态事件，不得影响主生成生命周期。
