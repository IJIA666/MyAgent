...
The system MUST discover skills from `~/.myagent/skills/` and `<workspace>/.myagent/skills/`. Each skill MUST be defined by a `SKILL.md` file with YAML Frontmatter and parsed using a robust, standard YAML parser to support complex/nested data types. Project skills MUST override user skills with the same skill name; different names MUST be merged. The system MUST NOT inject the full text of all skills by default; it MUST construct an `<available_skills>` index.

#### Scenario: Injecting Merged Skill Index
- **WHEN** the agent builds the system prompt
- **THEN** valid user and project skills are merged by name, project definitions win same-name conflicts, and one summarized `<available_skills>` index containing name and description is injected into the final prompt

#### Scenario: Tool-driven Full Skill Loading
- **WHEN** the agent identifies a need to use a skill from the `<available_skills>` index
- **THEN** the agent CAN invoke a designated internal Tool to load the full text from the resolved winning skill definition
