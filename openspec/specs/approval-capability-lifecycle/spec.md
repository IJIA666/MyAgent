...
系统必须（MUST）确保 `AgentLoop.chat()` 的大循环 `finally` 块及其 `RunEnd` Hook 管道中不包含任何 `clearTemporaryWhitelists()` 调用，消除旧行为中"每次迭代结束清空全部会话白名单"的缺陷。

#### 场景: 迭代结束后 session 白名单保持有效
- **WHEN** `AgentLoop.chat()` 的 while 循环完成一次迭代（包括所有工具调用和 LLM 推理），进入 finally 块触发 `RunEnd`
