...
提供 agent 向用户发起结构化提问并同步等待回答的能力，通过 `InteractionPort` 端口契约实现工具层与 CLI 交互层的解耦，与 `ApprovalPort` 在职责上严格分离。

## Requirements

### Requirement: 工具调用发起用户提问

agent 在推理过程中 MUST 能够通过调用 `ask_user_question` 工具向用户发起结构化提问并同步等待回答。该工具属于只读操作（`securityCategory: 'read'`），不受 Plan 模式工具裁剪影响。
...
- **WHEN** 用户在 CLI 展示提问后长时间未响应（默认 5 分钟超时）
- **THEN** 工具返回空字符串，agent 自行决定后续行为（如终止当前任务或使用默认策略继续）

#### Scenario: AbortSignal 取消处理

- **WHEN** 工具执行被外部 `AbortSignal` 取消（如 agent-loop 的工具超时熔断器触发、用户中断推理、会话销毁）
- **THEN** `InteractionPort.askUser()` 接收到 `signal` 的 abort 事件后立即终止等待，工具抛出 `AbortError` 或返回空字符串，CLI 层清理交互界面并恢复 InputListener。不得残留未清理的 stdin 监听器
...
- **THEN** 系统挂起当前工具执行，将 payload（title、options、multiSelect、allowFreeInput）传递给 CLI 层渲染交互界面，等待用户响应后返回用户回答字符串并恢复工具执行。若超时或用户取消，返回空字符串
