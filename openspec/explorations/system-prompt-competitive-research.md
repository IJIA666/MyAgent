...
本轮不再只比较摘要提示词，而是回到运行时源码，核对三个更容易造成数据丢失或无效压缩的问题：

1. 压缩前是否存在剪枝，剪枝后是否重新判断还需不需要压缩；
2. 自动压缩如何避免先做一次无效的中段摘要、随后立刻再做全量摘要；
3. 最近受保护尾部本身已经超过压缩目标时，系统如何退化；单条消息超过摘要输入窗口仅作为异常边界核对，不预设为正常场景。

调研对象仍为 `D:\projects\Agents` 下的七个参考项目。Tinypace 只暴露 Hermes 的压缩模型和上下文窗口配置，没有独立的上下文压缩算法，因此算法结论归入 Hermes，不将其虚构为第七套方案。

### H.2 竞品源码事实

#### H.2.1 Claude Code：先做粒度更小的回收，再决定是否全量压缩

`claude-code-analysis/src/query.ts` 的顺序是 snip、microcompact、context collapse、autocompact。注释明确说明：若前面的局部回收已经把上下文降到阈值以下，最后的 autocompact 应当 no-op，从而尽量保留细粒度历史。`autoCompact.ts` 还会把 `snipTokensFreed` 从当前估算中扣除，避免上一次 API usage 尚未反映本轮剪枝时误触发全量压缩。

`microCompact.ts` 会清理旧工具结果而保留近期结果；`compact.ts` 在摘要前剔除图片、文档和可重新注入的附件。若摘要请求本身返回 Prompt Too Long，则最多重试 3 次，每次按完整 API round 从头部删除旧输入，并插入明确的截断标记；只剩无法再删的输入仍超限时失败。

这套顺序支持“剪枝是语义摘要之前的低成本降级”这一判断，但 Claude Code 仍保留了窄范围的硬删除逃生阀。该逃生阀只用于让摘要请求能够发出，不是正常压缩层级。

#### H.2.2 OpenCode：请求前预测和单次溢出恢复较清楚，但没有活跃剪枝层

`opencode/packages/core/src/session/runner/llm.ts` 在正常模型请求前调用压缩预检；`session/compaction.ts` 估算 system、messages 和 tools 组成的完整请求，并为输出与安全 buffer 预留空间。若 provider 仍返回溢出，最多执行一次 `compactAfterOverflow`，恢复后的第二次请求不再循环压缩。

新实现会按最近 token 预算选择旧历史和近期历史，将旧摘要与近期序列化内容合并；摘要失败则不替换历史，也没有硬截断 fallback。配置 Schema 虽仍有 `prune` 字段，但当前 V2 `settings()` 没有读取它，不能把遗留配置当作正在运行的前置剪枝能力。

它的优点是“请求前预测 + 溢出后仅恢复一次”，能防止无限压缩循环；不足是选择逻辑主要基于序列化字符串和近似字符预算，不能提供协议级的单条消息分块保证。源码中超大单项的 prefix/suffix 拼接还存在疑似重复前缀的风险，应先用针对性测试确认，不能直接复制。

#### H.2.3 Hermes：确有压缩前历史剪枝，但缺少剪枝后重新判定

`hermes-agent/agent/context_compressor.py` 的 `_prune_old_tool_results()` 是不调用 LLM 的前置阶段：

- 对长工具结果计算内容 hash，只保留最新完整副本，旧重复结果替换为标记；
- 对保护尾部之外的旧工具结果移除图片，并生成按工具类型区分的短说明；
- 截短旧 assistant tool-call 参数时保持合法 JSON，避免破坏 provider 协议。

随后 `compress()` 保护头部，按 token 选择近期尾部，对中段做增量摘要。问题是剪枝完成后没有重新估算并在低于阈值时停止，而是继续生成摘要，因此仍可能做一次已经没有必要的 LLM 压缩。

Hermes 的尾部选择不切开消息，并保证最近 user 和 assistant 进入尾部。若最新 assistant 单条输出极大，它会被完整保护，中段压缩无法解决问题。系统通过“连续两次压缩均节省不足 10% 后停止自动尝试”来防止抖动，并建议新会话或定向压缩；这只能止损，不能恢复容量。

摘要失败策略也不完全安全：认证和网络错误会保留原历史，但部分配置下会生成静态 fallback 并丢弃中段。源码注释还记录过相关数据丢失问题，因此这种 fallback 不应作为 MyAgent 的默认行为。

#### H.2.4 OpenClaw：可选的请求投影剪枝，压缩可拆轮但不能拆消息

`openclaw/src/agents/agent-hooks/context-pruning` 提供独立于压缩的 context hook。它只在显式配置 `contextPruning.mode = cache-ttl`、且 provider/model 满足缓存 TTL 条件时注册，默认配置测试明确断言不启用该能力。

启用后，它不会修改持久化 transcript，而是在模型请求投影中：

- 保护首次 user 之前的 bootstrap 内容和最近若干 assistant 轮；
- 在 soft ratio 处把旧工具结果缩成 head + tail + 说明；
- 仍超 hard ratio 时，将更旧工具结果内容替换为占位符；
- 始终保留 tool-result 的消息结构和调用标识。

`packages/agent-core/src/harness/compaction/compaction.ts` 允许切点落在一轮内部：更早历史和该轮前缀分别摘要，保留该轮后缀。但切点仍位于消息之间，不会切开一条消息；摘要输入也没有分块。测试只覆盖两段摘要串联，没有覆盖单条消息超过摘要窗口。因此它能处理“一轮中有多条消息”的超大轮次，不能处理“单条 assistant 文本本身数十万 token”。

#### H.2.5 Codex：有结构化工具输出改写，也有激进的摘要输入硬删除

`codex/codex-rs/core/src/compact.rs` 的本地摘要若遇到 `ContextWindowExceeded`，会从摘要输入头部逐项删除最旧 history item 并重试；只剩一个输入仍超限时失败。这是明确的硬截断，而且没有固定删除次数上限。`pre_compact_hooks` 只能继续或停止，不能返回改写后的上下文，因此不是内置剪枝阶段。

远程压缩路径的 `trim_function_call_history_to_fit_context_window()` 更结构化：在估算仍超窗口时，把 function/custom tool 输出替换为截断标记，把 tool-search 工具列表清空，同时保留调用 ID 和成功状态。不过循环遇到第一个不可改写项就停止，只能处理尾部连续的可改写输出，不是通用历史剪枝器。

另一个 `compact_token_budget.rs` 路径跳过模型摘要，直接开启新上下文窗口并注入结构化 `WorldState`。这不是普通对话的无损压缩，而是“可靠外部状态存在时的换窗”。它对 Goal 模式有参考价值，但不能在没有完整结构化状态时静默替代对话历史。

#### H.2.6 Gemini CLI：工具结果先落盘再截短，失败后避免重复摘要

`gemini-cli/packages/core/src/context/chatCompressionService.ts` 会在切分历史前，对全部旧 functionResponse 应用反向 token 预算：最近工具结果优先共享 50K token，超预算的旧结果保存到项目临时目录，历史中保留尾部内容和文件引用；落盘失败则保留原结果，避免数据丢失。普通 user/assistant 文本不会被该阶段截断。

若自动摘要已经失败过一次，下一轮不再重复调用摘要模型，只使用工具结果截短后的历史；摘要成功后会重新计算完整新历史 token，若摘要反而膨胀则拒绝替换。它还会再调用一次模型校验摘要遗漏，质量更高但成本和延迟也更高。

新的 `contextCompressionService.ts` 进一步把旧 `read_file/read_many_files` 结果做 FULL、PARTIAL、SUMMARY、EXCLUDED 语义路由，保护最近 2 轮、缓存文件摘要，并在 functionCall/functionResponse 邻接关系损坏时整体回退原历史。这证明“按内容类型治理旧工具结果”是独立能力，而不是只能靠统一对话摘要；但它是编码工具特化，也没有解决单个超大普通 assistant 文本。

### H.3 横向结论

| 项目 | 压缩前剪枝 | 剪枝后重算 | 溢出恢复 | 单轮内部切分 | 单条消息切分 | 硬丢弃策略 |
| --- | --- | --- | --- | --- | --- | --- |
| Claude Code | 有，工具结果、附件和 collapse | 有，前序回收可让 autocompact no-op | 摘要 PTL 最多 3 轮 | 依具体 compaction 路径 | 无通用保证 | 仅摘要输入按旧 API round 删除 |
| OpenCode | V2 无活跃 prune | 不适用 | 最多恢复一次 | 序列化历史按预算选择 | 近似字符串切分，协议保证不足 | 无 fallback，失败保留历史 |
| Hermes | 有，重复/旧工具结果和参数 | **没有** | 失败冷却、低收益熔断 | 保护完整消息组 | 无 | 可选静态 fallback 会丢中段 |
| OpenClaw | 有，但默认关闭且仅请求投影 | hook 自身按 ratio 逐级处理 | 压缩失败返回错误 | 有，可摘要轮前缀 | 无 | 工具内容占位，不删持久历史 |
| Codex | 远程摘要请求有局部工具输出改写 | 有窗口估算 | 本地摘要持续删头重试 | 按 history item | 无 | 摘要输入可连续删除旧 item |
| Gemini CLI | 有，旧工具结果落盘/截短或语义路由 | 摘要后重新计数 | 失败后只用截短结果，避免再摘要 | 按 Content 边界 | 无 | 不删普通文本；工具原文有外部引用 |
| Tinypace | 无独立算法 | 不适用 | 继承 Hermes | 继承 Hermes | 继承 Hermes | 继承 Hermes |

没有一个参考项目把“单条普通消息自身超过摘要模型窗口”作为正常主路径处理。MyAgent 的摘要调用复用当前模型，在同一模型正常生成且输出上限受控时，assistant 单条输出通常不会超过该模型窗口；异常情况主要来自超大用户输入、模型切换、外部导入或序列化附加开销。因此首期不应为该边界建设复杂的单消息分块系统。当前更实际的风险是：最新完整轮次虽未超过模型窗口，却已经超过压缩后的目标预算，使中段摘要无法把请求降回安全水位。

### H.4 MyAgent 当前实现的具体缺口

`src/core/usecases/plugins/TokenWatermarkPlugin.ts` 当前只估算一次，超过阈值后直接调用 `compactInHook()`；`src/core/usecases/brain/CompactionService.ts` 又无条件完整保留最新 user 轮，即使该轮已经超过 `compactionRetainTokens`。两者组合会产生以下结果：

1. 已有的重复读取 Stub 和大工具输出预处理属于写入历史前的预防措施，不等于压缩触发时对既有历史的统一剪枝；
2. 没有剪枝后重算，无法在低成本回收已经足够时跳过摘要；
3. 没有压缩前收益预测，最新尾部自身超限时仍可能先做一次注定无效的中段摘要；
4. 中段摘要成功只代表历史被替换，不代表替换后的请求已低于阈值；当前插件也不验证实际节省量；
5. 若异常输入或附加开销仍让摘要请求超出窗口，目前没有受控失败协议；
6. 自动触发和未来手动全量压缩若各自独立实现，容易出现连续压缩两次、摘要再次摘要和状态漂移。

因此，前述“中段压缩后再降级为全量压缩”的顺序不够安全。决策必须发生在第一次 LLM 摘要之前，而不是用真实摘要调用试错。

### H.5 推荐方案：统一预算规划器，而不是串联两次压缩

推荐把自动与手动压缩都收敛到一个 `ContextBudgetPlanner`，按以下顺序执行：

1. **完整请求预估**：计入 system/developer、工具 Schema、历史、动态附件、目标模型最大输出和摘要预留，不再只估算 messages；
2. **结构化剪枝**：先处理可再获取、重复、过旧或超大工具结果，保留调用配对、错误状态、结果摘要和必要的外部引用；默认先生成请求投影，只有明确需要缩小持久历史时才提交有损改写；
3. **重新估算**：若已经回到安全阈值，直接发送请求，不生成摘要；
4. **压缩收益预测**：分别计算中段压缩和全量压缩的结果上界，包括受保护原文、摘要最大输出、固定前缀和输出 reserve；
5. **一次选择**：中段方案预计可满足目标才执行中段压缩；受保护尾部本身已超限、可摘要中段过小或预计收益不足时，直接选择全量/分块压缩，不能先中段再全量；
6. **结果验证**：替换前后都重新估算。若摘要膨胀或仍不满足目标，不提交中段结果；记录节省量和失败原因，避免下一轮重复同一无效动作；
7. **溢出恢复上限**：provider 仍报告窗口溢出时最多重新规划一次，第二次失败应明确返回，而不是无限重启。

首期“全量压缩”可以直接摘要全部非 system 历史，因为当前摘要复用同一模型，且水位应在摘要请求仍有输入、输出余量时提前触发。最新用户请求仅在受预算约束时保留原文，不能继续无条件保护整个最新轮。单消息分块可作为未来在模型切换、外部历史导入或真实溢出证据出现后的增强，不进入首期必做范围。

若 Goal 模式已经维护独立、可验证的目标和进度状态，全量压缩可把它作为恢复锚点；但结构化 Goal 只能补充对话事实，不能掩盖摘要失败或自动授权丢弃全部历史。手动压缩应复用同一规划器，可由用户显式要求 `middle` 或 `full`，而不是维护另一套物理算法。

### H.6 硬截断的推荐边界

“硬截断”需要区分三类行为，不能用一个布尔开关混在一起：

1. **结构化工具结果降级**：原文可重新读取或已安全落盘，历史中保留摘要、引用、调用 ID 和错误状态。这应属于正常剪枝，可以自动执行；
2. **摘要输入的应急窄化**：只影响本次摘要请求，不删除持久化原历史；按完整协议组删除最旧输入，插入明确标记，限制重试次数并记录遥测。这可以作为最后逃生阀；
3. **持久历史静默删除**：在没有摘要或外部引用时删除普通 user/assistant 内容。正常路径不应允许，摘要失败时也不应自动提交。

因此，不建议把所有工具结果缩减都称为“硬截断”，但首期也没有必要实现 Claude Code/Codex 式的普通历史删头重试。更稳妥的首期规则是：允许可恢复的结构化工具结果剪枝；禁止静默硬删持久普通对话；摘要请求仍超限时保留原历史并明确失败。只有真实运行证据证明早触发和结构化剪枝仍不足时，再单独评估不修改原历史、次数受限的摘要输入应急窄化。

### H.7 最终判断

本轮源码调研支持用户提出的核心方向，但需要把表述收紧：

- **剪枝应当是语义压缩之前的第一层降级**，且剪枝后必须重新估算；
- **全量压缩不是中段压缩失败后的盲目第二次调用**，而是规划器基于受保护区大小和预测收益提前选出的另一条路径；
- **首期不实现普通历史硬截断**；只允许可恢复的工具结果剪枝，异常溢出保持原历史并失败；
- **单条消息分块不是首期需求**；先通过提前水位、完整请求预算和结果验证解决正常路径；
- **Goal 模式应利用结构化状态降低恢复风险**，但不能依赖它掩盖上下文治理缺失。

下一步 OpenSpec 的合理边界不是简单增加一个 `fullCompact()`，而是先定义统一预算规划、结构化剪枝、一次性策略选择、结果验证和失败原子性，再让中段与全量成为规划器的两个执行策略。单消息分块和摘要输入硬截断暂不纳入首期。

### H.8 收敛后的首期最终方案

首期只建设一条统一决策链：

1. 在模型请求前估算完整请求预算，至少包含 system 前缀、工具 Schema、历史、动态附件和模型输出 reserve；
2. 对旧工具结果执行可恢复的结构化剪枝，优先复用现有落盘引用和 Stub 机制，不删除普通 user/assistant 文本；
3. 剪枝后重新估算，回到安全水位则直接继续，不调用摘要模型；
4. 仍超限时，在摘要前预测中段压缩后的固定头部、受保护尾部和最大摘要之和；预计能回到目标水位才选择中段压缩；
5. 受保护尾部已经过大或中段收益不足时，直接选择全量检查点摘要，不先执行一次中段压缩；
6. 压缩后重新估算，只有确实降低且回到目标水位才提交新历史；否则恢复原历史并返回失败；
7. 同一次模型请求最多执行一次压缩决策。provider 仍报告溢出时只允许重新规划一次，禁止循环压缩。

首期保留现有中段压缩语义，新增全量检查点策略和统一 planner；不做单消息分块、不做普通历史硬截断，也不先引入复杂的多级摘要。手动压缩复用同一 planner，并允许显式请求 full；Goal 模式可以把已有结构化目标状态作为全量摘要输入的一部分，但不另建独立算法。
