...
| 2026-08-08 | Skill Review/Curator 统一经公共子代理运行器（unify-skill-review-runtime）完成归档：删除 background-skill-review.ts 手搓 AgentLoop 装配（约 110 行），Review/Curator 统一走 SubagentRuntime.runTask；subagentRuntime 必填 + SessionManager 装配期缺失抛错；全门禁通过（1309 单测 + 135 契约） | 评审修正：① 遗漏 4 处旧路径测试构造点（SessionManager.test 1287/1453 实际调用 runReview、1164 与 isolation freshSession 创建无运行器服务）——分流为注入运行器或外部 scheduler；② 检查位置前移：装配抛错必须早于 RuleManager 向 SkillLibrary.subscribe 注册监听（防订阅引用遗留），补直接测试断言 subscribe 未调用；③ Options 接口删 6 字段 + 必填属构造契约变更（改表述为"运行行为不变、内部装配契约有意变更"）；④ 断言失败原则：先判运行器是否违反隔离规格，禁止反向放宽规格断言 |
| 2026-08-08 | 后台记忆巩固（memory-consolidation）完成归档：对齐 Claude Code Auto Dream——时间(24h)+会话(5)双门控、时间状态与互斥锁分离（复用 CrossProcessLockManager）、fork(exact-fork) 四阶段整理（Orient/Gather/Consolidate/Prune）、`/memory-dream` 手动入口、`memory_dream_update` 展示事件；全门禁通过（1340 单测 + 135 契约） | 两轮 GPT 评审修正：① 锁语义（PID 重读竞态 → 复用 wx+token 跨进程锁；回收条件 AND 语义与现有实现一致）；② 会话快照格式（JSONL → session_<id>.json）；③ 执行载体（fresh → exact-fork 对齐官方 fork，父工具展示 + callTool 执行限制 + 控制文件保护）；④ 真实链路闭环（status 返回 failed/cancelled 非异常、冻结会话列表入队、mutationHook 不挂载改 ToolView 内收集、AgentEvent+CLI 渲染、动态开关装配、手动任务登记、null 回滚改删文件） |
