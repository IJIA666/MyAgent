...
- **THEN** 返回该工具对应的 `(args, cwd) => SafetyResource[]` 函数
...
- **WHEN** 定义一个文件写入类工具
- **THEN** 其 `resourceExtractor` 函数接收 `(args, cwd)`，返回读取文件路径并标记为 `{ kind: 'path', access: 'write', normalizedPath }` 的 `SafetyResource` 数组

#### 场景: 文件只读工具声明其资源提取器

- **WHEN** 定义一个文件只读类工具
- **THEN** 其 `resourceExtractor` 函数接收 `(args, cwd)`，返回标记为 `{ kind: 'path', access: 'read', normalizedPath }` 的资源
