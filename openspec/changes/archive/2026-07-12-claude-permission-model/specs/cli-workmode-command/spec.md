...
### Requirement: Legacy WorkMode CLI Commands

**Reason:** CLI 不再暴露旧 `Safe`、`Auto`、`YOLO`、`Plan` 工作模式语义。
...
#### Scenario: User switches to Plan from another mode

- **WHEN** 用户在 CLI 选择 `Plan`
- **THEN** 系统 MUST 通过统一模式转换入口进入 `plan` 并保存 `prePlanMode`

#### Scenario: User switches to Auto
