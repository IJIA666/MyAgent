...
│  职责: 根据受信任的 choiceId 生成 grant/effect，              │
│       但不在 Hook 忙区直接修改 SessionContext               │
│  输出: PendingGrant | PersistentRuleEffect | deny          │
├─────────────────────────────────────────────────────────┤
│  安全提交层 (AgentLoop / 持久化适配器)                      │
│  职责: 在 Hook 管线安全结束后提交会话级授权，或写入持久规则   │
│  输入: PendingGrant | PersistentRuleEffect                │
└─────────────────────────────────────────────────────────┘
```
...
3. **choiceId 推导**：根据 WorkMode、资源类型、已有规则计算可用的 choiceId 列表，`deny` 必须作为显式终态语义参与建模，而不是留给 UI 自行约定。
...
interface ApprovalChoice {
  ...
  label: string;         // 如 "单次放行" / "本次会话始终放行" / "永久放行" / "拒绝"
  description?: string;
}
...
```typescript
type ApprovalEffect =
  | { type: 'call'; toolName: string; resources: SafetyResource[] }
  | { type: 'session'; resources: Array<{ access: 'read' | 'write'; normalizedPath: string }> }
  | { type: 'persistent-command-prefix'; prefix: string }
  | { type: 'deny' };
...
function mapChoiceToEffect(
  ...
  switch (choiceId) {
    case 'call':
      return { type: 'call', toolName, resources: operation.resources };
    case 'session': {
      ...
      return { type: 'session', resources };
    }
    case 'persistent': {
      const prefix = operation.resources.find(
        (r): r is SafetyResource & { kind: 'command-prefix' } => r.kind === 'command-prefix'
      );
      if (!prefix) {
        return { type: 'deny' };
      }
      return { type: 'persistent-command-prefix', prefix: prefix.prefix };
    }
    case 'deny':
      return { type: 'deny' };
  }
...
`HumanApprovalPlugin` 在 `BeforeTool` Hook 中只能把授权结果映射为 `pendingGrant` 或持久化 effect，不能直接调用 `session.addTemporaryReadWhitelist()` / `session.addTemporaryWriteWhitelist()`。原因是当前 `SessionContext` 在 Hook 管线执行期间处于 `isProcessing` 忙锁保护中，直接修改会触发异常。

因此，正确链路必须是：

1. `checkSafety()` 返回 `SafetyOperation`
2. `ApprovalPolicy` 生成 `ApprovalRequest`
3. UI 返回 `choiceId`
4. `HumanApprovalPlugin` 将 `choiceId` 映射为 `pendingGrant`
5. `AgentLoop` 在 Hook 管线成功结束且 `toolCallId` 匹配后，安全提交 `call/session` 级授权
6. `persistent` 仅对受信任的 `command-prefix` 生效，并通过独立持久化路径写入规则
