...
### 需求: 目录浏览型只读资源必须显式声明为目录范围资源

系统必须支持一种显式的目录范围只读资源类型，用于表达"允许读取某目录及其子树"，并与普通精确路径资源区分。

#### 场景: 目录浏览工具上报 directory-scope 资源
...
### 需求: 目录范围读授权必须对子树生效，但不得扩展到兄弟目录或写权限
...
### 需求: 本 change 仅覆盖 listFiles 的目录浏览语义

系统必须将本次目录范围授权能力限定在 `listFiles` 对应的目录浏览场景，不得在没有单独设计与验证的前提下自动扩展到其他搜索类工具。

#### 场景: grepSearch 不因本 change 自动获得目录范围语义

- **WHEN** 系统处理 `grepSearch` 的资源声明与授权
- **THEN** 行为必须保持现状

#### 场景: globSearch 不因本 change 自动获得目录范围语义
