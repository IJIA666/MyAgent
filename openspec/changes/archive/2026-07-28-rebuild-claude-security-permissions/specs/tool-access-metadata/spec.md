...
**Reason:** `(args) => SafetyResource[]` 与宽泛资源字典无法表达 provenance、可信度和操作节点。

**Migration:** 使用 `Typed Authorization Resource Evidence`。

### Requirement: 工具自带元数据声明

**Reason:** 原可选元数据允许 effectful 工具遗漏资源契约。

**Migration:** effectful 工具必须注册完整权限适配器和正式证据构造器。

### Requirement: 访问元数据端口契约

**Reason:** 原端口只提供可选 extractor 查询，不能形成穷尽覆盖。

**Migration:** ToolCatalog 暴露可枚举的正式适配器和证据契约。
