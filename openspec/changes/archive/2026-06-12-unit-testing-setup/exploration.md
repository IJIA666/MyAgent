...
- **业界主流实践调研**：
  ...
  - **关于单元测试提交 Git 的工程共识**：所有的单元测试源码必须提交至 Git 仓库中。单元测试充当了“活的接口规范文档”，并且是 CI/CD 流水线执行自动化门禁的必要输入。测试生成的本地覆盖率报告（如 `coverage/`）以及本地私有配置文件则必须排除。
...
| **开发体验** | 拥有基础的 `--watch` 监听模式 | 极速热更新、丰富的错误堆栈以及可视化 UI 界面 ✓ | 方案 B 占优 |
