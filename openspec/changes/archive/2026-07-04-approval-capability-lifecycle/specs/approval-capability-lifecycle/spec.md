...
### 需求: session 型 PendingGrant 包含 toolCallId（ADDED）
...
### 需求: Tail call 工具调用走完整工具调用生命周期（ADDED）
系统必须（MUST）为 tail call 生成独立的 `toolCallId`，并走完整的 `callTool` → beforeTool 管线 → pendingGrant 提交 → 令牌生命周期。

#### 场景: Tail call 获得独立 toolCallId
- **WHEN** `agent-loop.ts` 执行 tail call（第 693 行）
- **THEN** 系统必须为 tail call 生成独立的 `toolCallId`，传入 `toolRegistry.callTool()`，与主调用（第 642 行）行为一致
...
### 需求: hasClaimedResource 必须校验 access 维度（ADDED）
...
### 需求: NativeTool.execute 接口契约迁移（ADDED）
...
### 需求: DeletePathTool 收敛到统一安全模型（ADDED）
