...
系统必须在安全检查时按资源类型和优先级链依次判定授权状态，高优先级作用域命中后不再检查低优先级。对于路径资源，优先级链至少应满足 `call capability → session 临时白名单 → 触发审批`；对于命令前缀资源，还必须保留 `persistent` 持久白名单的后续判定。

检查优先级链：
- 路径资源：`call capability 令牌 → session 临时白名单 → 触发审批`
- 命令前缀资源：`call capability 令牌 → session 临时白名单（若适用） → persistent 持久白名单 → 触发审批`

#### 场景: call capability 令牌优先于 session 白名单
- **WHEN** 当前工具调用同时存在有效的 `call` capability 令牌和匹配的 `session` 白名单
- **THEN** 系统必须先检查 call capability 令牌，命中后直接放行，不再检查 session 白名单

#### 场景: session 白名单优先于 persistent 白名单
- **WHEN** 当前操作同时匹配 session 临时白名单和 persistent 持久白名单
- **THEN** 系统必须优先使用 session 白名单判定
- **AND** session 白名单在会话关闭时随 SessionClosed 清理，不影响 persistent 的跨会话持久性
