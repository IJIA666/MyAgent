...
### 需求: 时间窗口流量聚合与接口输出
监控服务必须(MUST)在内存中实时订阅 ETW 网络事件并累计流量。当客户端（Agent）调用查询工具时，服务必须(MUST)按照设定的时间窗口（例如过去 5 秒）进行聚合，计算速率（Bytes/sec），且仅返回上传或下载流量排名前 N 的精简 JSON 报告。
...
监控服务必须(MUST)监听 `Microsoft-Windows-Kernel-Process` ETW 事件，动态跟踪进程的创建和销毁，并在内存中维护 PID 到进程信息的 LRU 缓存，以保证流量瞬发进程能够被准确追溯进程名。
...
### 需求: 系统性能计数器诊断
监控服务必须(MUST)提供暴露系统 CPU、内存和磁盘 I/O 精细度指标的工具，包括内核态/用户态 CPU 时间、页面错误率、非分页池大小、物理/虚拟内存提交以及磁盘排队长度。
