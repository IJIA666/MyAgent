...
### Requirement: 所有斜杠命令 SHALL 以 stdin 独占事务执行 [Amend 修正 — 替换原 setImmediate 方案]
...
#### Scenario: 菜单导航期间防止 stdin 所有权竞争 [Amend 修正 — 从 / 菜单扩大到所有斜杠命令]
...
#### Scenario: showInteractiveMenu / dispatchCommand 异常后监听器恢复 [Amend 新增]
...
#### Scenario: handleUserInput 同步抛错后监听器不卡死 [Amend 新增]
...
### Requirement: InputListener 的 pending resume 与延迟 line 事件 SHALL 可取消 [Amend 修正]
...
#### Scenario: 旧 readline 延迟 line 事件在新 start() 后穿透 [Amend 新增]
