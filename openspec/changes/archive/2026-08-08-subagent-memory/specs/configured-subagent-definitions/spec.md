...
系统 MUST 解析 `.md` 定义 frontmatter 并使以下字段生效：`description`（必填）、`tools`（显式工具名单）、`disallowedTools`（剔除名单）、`model`、`maxTurns`、`permissionMode`（仅允许收窄到 `plan` 或保持父模式，不得提升）、`omitClaudeMd`、`mcpServers`（字符串引用或内联定义，见 `subagent-agent-mcp`）、`initialPrompt`（`--agent` 主会话首轮前缀，见 `agent-session-mode`；子代理执行路径不消费）、`background`（布尔；声明 `true` 时模型调用该类型强制后台执行，声明 `false` 或省略时保持既有前后台语义）、`memory`（`user`/`project`/`local` 三值，见 `subagent-memory`；声明非法值时拒绝定义）。字段 `effort`、`color`、`skills`、`hooks`、`isolation` 属于本阶段未启用字段：MUST 被解析但 MUST NOT 生效，MUST 记录一条可诊断 warning，MUST NOT 导致文件被拒绝。`background` 非布尔值时 MUST 拒绝该定义并记录可诊断日志（fail-closed，对齐 tools/model/maxTurns 处理风格），MUST NOT 静默忽略。`memory` 声明非三值字符串或非字符串时 MUST 拒绝该定义并记录可诊断日志（fail-closed），MUST NOT 静默忽略。`tools` 声明恰好为 `['*']` 时 MUST 归一化为未声明（默认池语义，避免通配符过滤掉全部工具）。自定义定义 MUST 使用 `fresh` 上下文策略，`.md` 不得声明上下文策略字段。`.md` 正文 MUST 作为子代理系统提示。
...
- **WHEN** 定义 frontmatter 声明 `effort: high` 或 `hooks: {...}`
- **THEN** 该字段不产生任何运行时行为
- **AND** 系统记录一条 warning 指明该字段将在后续阶段启用

#### Scenario: memory 声明生效

- **WHEN** 定义 frontmatter 声明 `memory: project`
- **THEN** 该字段按 `subagent-memory` 契约生效（作用域解析、启动注入、权限冻结）
- **AND** 不再记录"未启用字段"warning

#### Scenario: 非法 memory 值拒绝定义

- **WHEN** 定义 frontmatter 声明 `memory: team` 或 `memory: true`
- **THEN** 该定义被拒绝且记录可诊断日志
- **AND** 不影响其他合法文件的加载
