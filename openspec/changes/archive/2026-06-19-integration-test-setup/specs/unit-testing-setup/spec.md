...
#### 场景: 物理配置自动引导与生成集成校验
- **WHEN** 开发者在临时沙箱目录中物理清空配置文件，并运行引导函数 `ensureConfigFiles()` 及 `loadConfig()`
- **THEN** 系统必须在磁盘上自动从 `.example` 拷贝生成物理文件，且读取还原出的物理配置参数准确无误

#### 场景: MCP 真实子进程敏感环境变量隔离物理审计
- **WHEN** 开发者执行集成测试，物理 `spawn` 启动自检的子进程，并模拟宿主机注入包含敏感大模型 Key 在内的环境变量时
- **THEN** 捕获并解析该子进程的环境，其输出必须包含 PATH 等基础变量，但决不能包含大模型 API Key（`AGENT_LLM_API_KEY`）等安全白名单外的敏感数据
