...
定义子代理模型的解析契约：优先级链（env > Agent 工具 `model` 参数 > 定义 frontmatter `model` > `inherit`）、同 tier 防降级（父已用某模型 tier 时子代理 alias 继承父精确模型）、`inherit` 的运行时解析，以及 exact-fork 始终继承父模型、不接受 model 参数的边界。对齐官方 `utils/model/agent.ts` 的 `getAgentModel` 语义。
...
系统 MUST 将解析得到的 profile ID 通过 `getModelConfig` 构造完整冻结 `LlmConfig`（含该 profile 的环境变量、baseUrl、contextWindow、temperature 与超时等完整档案），而非仅替换父配置的 model 字符串。`inherit` MUST 沿用父会话 `LlmConfig`。MyAgent 无 Claude 式族别名与 tier 体系，官方 `aliasMatchesParentTier` 语义不在本阶段落地。
...
系统 MUST 使 `Agent` 工具 schema 声明可选 `model` 参数（裸族别名或具体模型字符串），并对非法值返回可诊断校验错误。exact-fork 语义下 `model` 参数 MUST NOT 生效：fork 始终继承提交时父模型配置，调用侧传入的 `model` 被忽略（fork 开关开启时 schema 不暴露该参数，与 `run_in_background` 一致）。
