...
export class LoopPreventionPlugin implements Plugin {
  ...
  private loopPreventionLimit: number;
  ...
  constructor(appConfig?: AppConfig) {
    this.loopPreventionLimit = appConfig ? appConfig.runtimeLimits.loopPreventionLimit : 3;
  }
  ...
  public readonly hooks = {
    ...
    [HookEventName.BeforeTool]: async (context: HookContext, next: () => Promise<void>) => {
      ...
      if (toolCall) {
        ...
        if (callCount >= this.loopPreventionLimit) {
          context.control = {
            action: 'abort',
