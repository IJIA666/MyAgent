 ...
 * 使用真实 src/utils/logger.ts 的 initLogger()、configureFileSink()、结构化属性写入和 LogTape 文件 sink，
...
describe('日志管道合约测试 — 真实 initLogger / configureFileSink', () => {
  ...
  it('未配置文件 sink 时日志系统不产生文件', async () => {
    ...
    try {
      ...
      // 没有调用 configureFileSink，任何目录下都不应有 run.log
      expect(existsSync(join(tmpDir, 'logs', 'run.log'))).toBe(false);
      expect(existsSync(join(tmpDir, '.myagent', 'run.log'))).toBe(false);
    } finally {
      if (existsSync(tmpDir)) rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it('结构化事件应包含 component/event/sessionId 字段，内部名称不进入前台', async () => {
    ...
    try {
      vi.resetModules();
      const { initLogger, configureFileSink, disposeLogger, logger, LOG_COMPONENT } = await import('../../src/utils/logger.js');
      await initLogger();
      await configureFileSink(logDir);
  ...
  it('无内容变化的技能事件应为 DEBUG 级别，内部名称不进入前台 console', async () => {
    ...
    try {
      vi.resetModules();
      const { initLogger, configureFileSink, disposeLogger, logger, LOG_COMPONENT, LOG_EVENT } = await import('../../src/utils/logger.js');
      await initLogger();
      await configureFileSink(logDir);
